...
def drop_token_old(game_board, open_row, col_choice, token):
        ...
        if event.type == pygame.MOUSEBUTTONDOWN:
            ...
            if turn == 0:
                if flag_val == 0:
                    ...
                    flag_val = 1#int(input("Player 1's Turn to choose the column(0-6) to insert the token"))
                if check_isValid_Location(game_board, col_choice):
                        #open_row = get_next_open_row(game_board, col_choice)
                        response = gm.check_winning_recursive(game_board, 1, col_choice)
                        col_choice = None
                        print(response)
                        drop_token_old(game_board, response[0], response[1], 1)
            ...
            #Player 2's game
                ...
                if check_isValid_Location(game_board, col_choice):
                    open_row = get_next_open_row(game_board, col_choice)
                    drop_token_old(game_board, open_row, col_choice, 2)
